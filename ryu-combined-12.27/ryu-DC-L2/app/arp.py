...
class IcmpResponder(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ip_addr = ['10.108.91.100', '10.108.92.100']
...
#'type': 'ArpMessage', '__class__': 'arpevent'}
    ...
    def _handle_arp(self, datapath, port, pkt_ethernet, pkt_arp):
        if pkt_arp.opcode != arp.ARP_REQUEST:
            return
        if pkt_arp.dst_ip in self.ip_addr:
            pkt = packet.Packet()
            pkt.add_protocol(ethernet.ethernet(ethertype=pkt_ethernet.ethertype,
                                    dst=pkt_ethernet.src, src=self.hw_addr))  # src and dst mac addresses
            index = self.ip_addr.index(pkt_arp.dst_ip)
            pkt.add_protocol(arp.arp(opcode=arp.ARP_REPLY,
                                     src_mac=self.hw_addr, src_ip=self.ip_addr[index],
    ...
    def _handle_icmp(self, datapath, port, pkt_ethernet, pkt_ipv4, pkt_icmp):
        if pkt_icmp.type != icmp.ICMP_ECHO_REQUEST:
            return
        if pkt_ipv4.dst in self.ip_addr:
            index = self.ip_addr.index(pkt_ipv4.dst)

            pkt = packet.Packet()
            pkt.add_protocol(ethernet.ethernet(ethertype=pkt_ethernet.ethertype,
                                           dst=pkt_ethernet.src, src=self.hw_addr))
            pkt.add_protocol(ipv4.ipv4(dst=pkt_ipv4.src, src=self.ip_addr[index],
