...
class DomainInfo(object):
    ...
    def __init__(self, domainId, *args, **kwargs):
        ...
        self.linkPort = []#src, srcPort, dst, dstPort
        self.wsgiIp = None
        self.wsgiPort = None
        self.virPort = []#vir dpid
        self.enterTime = time.time()
    ...
    def addvirPort(self, dpid):
        if dpid not in self.virPort:
            self.virPort.append(dpid)

    def getSwitchFeature(self, node):
        assert node in self.switchesFeatures
        return self.switchesFeatures[node]

    def addSwitchFeature(self, node, features):
