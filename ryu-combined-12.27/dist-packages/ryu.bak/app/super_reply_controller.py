...
class SuperReplyController(object):
    ...
    def switchEnter(self, jsonMsg, SC):
        ...
        self.logger.info("Switch %d enter in global topo" % node)
    ...
    #                                                                                    dstSwitch, dstPort))
        ...
        if (srcSwitch, dstSwitch) not in topo.edges():
            ...
            self.logger.info("Global topo link add:src( %d, %2d)-> dst( %d, %2d)" % (srcSwitch, srcPort,
                                                                                       dstSwitch, dstPort))
    ...
    ###############################################admin code
    # def datapathAddressMessage(self,jsonMsg,SC):
    #     assert jsonMsg[TYPE] == 'datapathAddressMessage'
    #     send_administrator = jsonMsg['send_administrator']
    #     controller_ip_port = jsonMsg['controller_ip_port']
    #     SC.table.openflow_version.update({'openflow_version':send_administrator['openflow_version']})
    #     SC.table.datapath_address.update({'datapath_address':send_administrator['datapath_address']})
    #     SC.table.packetIn_num = send_administrator['packetIn_num']
    #     SC.table.packetOut_num = send_administrator['packetOut_num']
    #     SC.table.packetIn_message = send_administrator['packetIn_message']
    #     SC.table.controller_ip_port = controller_ip_port
    #     SC.table.app_name = {controller_ip_port.keys()[0]: jsonMsg['app_name']}
    ###############################################admin code
...
###################################################################################################
    def pathReport(self,jsonMsg,SC):
        ...
        # print "domainPathList:" , domainPathList
        # print "virTopo.edges:", virTopo.edges()
        # print "virTopo.nodes:", virTopo.nodes()
        # print "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
