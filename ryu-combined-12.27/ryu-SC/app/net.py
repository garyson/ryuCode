...
######################################################################################
    ...
    def taskSetIpFields(self, srcSwitch=None, dstSwitch=None, srcIp=None, dstIp=None, local_mac=None,
                      next_mac=None, last_outport_num=None, bandwidth=None, nexthop=None):
        self.srcSwitch = srcSwitch
        self.dstSwitch = dstSwitch
        self.srcIp = srcIp
        self.dstIp = dstIp
...
#############################################################################################
    ...
    def getVirSectorialPath(self, nodeTodomain,virEdges):
        ...
        virEdges1 = []
        for i, j in virEdges:
            if nodeTodomain[i] == nodeTodomain[j]:
                virEdges1.append((i,j))


        for in_port, out_port in virEdges1:
            nodeDomain = nodeTodomain[in_port]
            # print nodeDomain
            # print nodeTodomain[out_port]
...
###############################################################################################
    ...
    def makeDomainTaskAssign(self, domainId, virSectorialPath, type='main'):
        to_send = {}
        ...
        if type == 'main':
            to_send[PARTPATH] = virSectorialPath.get(domainId)#return a dict
